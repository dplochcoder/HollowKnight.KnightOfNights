...
[PlandoSubmodule]
internal class ElderHuModule : AbstractGhostWarriorModule<ElderHuModule>
{
    ...
    protected override void ModifyGhostWarrior(PlayMakerFSM fsm, Wrapped<int> baseHP)
    {
        ...
        fsm.GetFsmState("Place Rings").AddLastAction(new Lambda(() =>
        {
            ...
            {
                if ((phase3.Value && Random.Range(0, 4) == 0) || (phase2.Value && Random.Range(0, 3) != 0) || (!phase2.Value && !phase3.Value))
                {
        ...
        megaState.AddLastAction(new Lambda(() =>
        {
            ...
            if (!phase3.Value || choice == 0)
            {
                IEnumerator Standard()
                {
                    for (int i = 0; i < 9; i++)
                    {
                        ...
                        yield return new WaitForSeconds(i == 8 ? (phase3.Value ? 0.65f : (phase2.Value ? 0.75f : 1f)) : (phase3.Value ? 0.1f : (phase2.Value ? 0.15f : 0.3f)));
                    }
            ...
            else
            {
                ...
                IEnumerator DubStep()
                {
                    ...
                    for (int j = 0; j < 5; j++)
                    {
                        for (int i = 0; i < 17; i++) if ((i + j) % 2 == off) controls[i].gameObject.SetActive(true);
                        PlayMegaRingSounds(fsm, 0.3f);
    ...
    protected override ElderHuModule Self() => this;
}
