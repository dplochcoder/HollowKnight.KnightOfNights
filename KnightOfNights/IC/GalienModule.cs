...
[PlandoSubmodule]
internal class GalienModule : AbstractGhostWarriorModule<GalienModule>
{
    ...
    protected override void ModifyGhostWarrior(PlayMakerFSM fsm, Wrapped<int> baseHp)
    {
        ...
        {   
            ...
            yield return new WaitUntil(() => UpdatePhase(fsm, baseHp, mini1, 0.9f));
            yield return fsm.StartCoroutine(SummonMiniHammer());

            Wrapped<bool> mini2= new(false);
            yield return new WaitUntil(() => UpdatePhase(fsm, baseHp, mini2, 0.75f));
            yield return fsm.StartCoroutine(SummonMiniHammer());
    ...
    protected override GalienModule Self() => this;
}
